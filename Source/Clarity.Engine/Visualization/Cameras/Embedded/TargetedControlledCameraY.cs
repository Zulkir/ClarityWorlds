...
{
    public class TargetedControlledCameraY : IBoundControlledCamera
    {
        #region Props struct
        public struct Props
        {
            ...
            {
                var forward = -Vector3.UnitZ * Matrix3x3.RotationX(Pitch) * Matrix3x3.RotationY(Yaw);
            ...
            public CameraFrame GetFrame()
            {
                ...
                viewFrame.Forward = -Vector3.UnitZ * Matrix3x3.RotationX(Pitch) * Matrix3x3.RotationY(Yaw);
                viewFrame.Right = Vector3.Cross(viewFrame.Forward, Vector3.UnitY).Normalize();
        ...
        public TargetedControlledCameraY(ISceneNode node, Props initialProps)
        {
            this.node = node;
        ...
        public CameraFrame GetGlobalFrame() { return GetLocalFrame() * Node.GlobalTransform; }
        ...
        private bool TryHandleMouse(IMouseEventArgs eventArgs)
        {
            if (eventArgs.ComplexEventType == MouseEventType.Move)
            {
                if (eventArgs.State.Buttons == MouseButtons.Left)
                {
                    var scale = 0.01f;
                ...
                if (eventArgs.State.Buttons == MouseButtons.Right)
                {
        ...
        public static TargetedControlledCameraY Default(ISceneNode node)
        {
            return new TargetedControlledCameraY(node, Props.Default);
        }
    }
}