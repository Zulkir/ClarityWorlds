...
namespace Clarity.Core.AppCore.StoryGraph.Editing.Flowchart
{
        ...
        IFocusNodeComponent
    {
        ...
        protected StoryFlowchartEditSceneComponent()
        {
            ...
            routeMaterial = new StandardMaterial(new SingleColorPixelSource(Color4.White))
            {
                IgnoreLighting = true,
                LineWidth = 3
            };
        ...
        private void UpdateRouteEdges(IStoryGraph storyGraph)
        {
            ...
            foreach (var key in allRouteEdges.Where(x => !edgeGizmos.ContainsKey(x)))
                AddNewEdge(key, routeMaterial);
        }

        private void AddNewEdge(UnorderedPair<ISceneNode> key, IStandardMaterial material)
        {
