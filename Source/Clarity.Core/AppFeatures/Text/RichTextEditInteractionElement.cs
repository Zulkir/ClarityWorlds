...
namespace Clarity.Core.AppFeatures.Text
{
    public class RichTextEditInteractionElement : IInteractionElement
    {
        ...
        public RichTextEditInteractionElement(IRichTextComponent cText, IInputHandler inputHandler, IUndoRedoService undoRedo, IClipboardService clipboardService)
        {
            this.cText = cText;
            ...
            if (args.ComplexEventType == KeyEventType.Down)
            {
                switch (args.EventKey)
                {
                    ...
                    case Key.Tab:
                        ...
                        return true;
                    case Key.C:
                        if (args.KeyModifyers != KeyModifyers.Control || !cText.SelectionRange.HasValue)
                            return true;
                        Copy(cText.SelectionRange.Value);
                        return true;
                    case Key.X:
                        if (args.KeyModifyers != KeyModifyers.Control || !cText.SelectionRange.HasValue)
                            return true;
                        ...
                        return true;
                    case Key.V:
                        if (args.KeyModifyers != KeyModifyers.Control || !clipboardService.TryGetString(out var str))
                            return true;
                        Paste(str);
                        return true;
                }
        ...
        private bool TryHandleMouseEvent(IMouseEventArgs args)
        {
            ...
            var toolReceiverAspect = cText.Node.PresentationInfra().PlacementNodeAspect;
            if (toolReceiverAspect == null)
                return false;
            var globalRay = args.Viewport.GetGlobalRayForPixelPos(args.State.Position);
            if (!toolReceiverAspect.PlacementPlane.TryFindPoint2D(globalRay, out var point2D))
                return false;
        ...
        private void Copy(RtRange range)
        {
            var text = cText.TextBox.Text;
            ...
            clipboardService.CopyString(builder.ToString());
        }
