...
namespace Clarity.Core.AppFeatures.StoryLayouts.NestedSpheres
{
    public class NestedSpheresStorySpringModel
    {
        ...
        private static float CalcMinRadiusForChildren(int numChildren, float childRadius)
        {
            ...
            var boundingSphere = Sphere.BoundingSphere(positions.Select(x => new Sphere(x, childRadius)).ToArray());
            ...
            foreach (var child in orderedChildren.Skip(1))
        {
            ...
            var childSpheres = children.Select(x => new Sphere(positions[x], radii[x])).ToArray();
            var boundingCircle = Sphere.BoundingSphere(childSpheres);
            foreach (var child in children)
        ...
        private void MoveWhilePossible(int node, Vector3 offset)
        {
            ...
            var spheres = children
                //.Where(x => placed[x])
                .ExceptSingle(child)
                .Select(x => new Sphere(positions[x], ownRadius + radii[x]))
                .ToArray();
