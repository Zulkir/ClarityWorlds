...
namespace Clarity.Ext.Rendering.Ogl3.Caches
{
    public unsafe class RawDataResCache : AutoDisposableBase, ICache
    {
        ...
        public IBuffer GetGlVertexBuffer() => 
            GetGlBuffer(ref glVertexBuffer, BufferTarget.Array, 0);

        public IBuffer GetGlIndexBuffer(int offset)
        {
            if (offset == 0)
                return GetGlBuffer(ref glIndexBuffer, BufferTarget.ElementArray, 0);
            ...
            var modifyed = old;
            modifyed = GetGlBuffer(ref modifyed, BufferTarget.ElementArray, offset);
            if (modifyed != old)
